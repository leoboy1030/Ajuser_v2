...
await message.client.sendMessage(message.jid, fs.readFileSync('.najnvannu.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio,duration: 99978509, quoted : message.data, ptt: true})
        }
...
if(pattern.test(message.message)){
    ...
    Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {   
        ...
        if(banned !== undefined) return
        if (!!message.mention && message.mention[0] == '918281794574@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('.najnvannu.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio,duration: 99978509,contextInfo: { forwardingScore: 3, isForwarded: true }, quoted : message.data, ptt: true})
        }
        if (!!message.mention && message.mention[0] == Config.AFNN) {
...
const array = ['aju','Bye','bye','Poli','Dance','Gd mrng','athulmachan','devarayoli','ebulljet','hiya hiya','judumonbgm','judumonmass','judumonrecord','judumonshambu','Mangalassery Neelakandan','najnvannu','nenban','ooohhh','pinnalah','porasyma','power','pronoob','sedlyf','sedlyf2','SIA','siiuuuuu','sonnetleo10','sonnetleo30Ringtone','velechil','sed','Ara','Poyi','myr','poda','podi','Pain','girl','asena','ayin','back','Back','Bot','fuck','Fuck','Hehe','Hello','Hlo','Kill','kill','kiss','line','love','mwolu','Mwolu','single','tha','thund','z','Z','bie','Bie','Ajuser']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
